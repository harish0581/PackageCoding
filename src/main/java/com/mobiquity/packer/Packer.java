...
// Parsing the input file an storing them into the List collection
...
// Use to separate the weight and cost to check the matching string from the input against the actual
...
// finding the best combination that matches with the weight and cost
    private static String getBestCombinationAsString(int maxWeight, List<Package> packages) {
        int noOfPackages = packages.size() + 1;
        int weights = maxWeight + 1;
        double[][] a = new double[noOfPackages][weights];
        ...
        int j = maxWeight;
        double totalcost = a[noOfPackages - 1][weights - 1];
        for (; j > 0 && a[noOfPackages - 1][j - 1] == totalcost; j--);

        for (int i = noOfPackages - 1; i > 0; i--) {
            if (a[i][j] != a[i - 1][j]) {
